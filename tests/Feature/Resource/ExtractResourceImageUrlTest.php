...
class ExtractResourceImageUrlTest extends TestCase
{
    ...
    public function it_extracts_resource_image_urls_and_stores_them()
    {
        ...
        $this->assertEquals('https://example.com/image1.png', DB::table('resources')->where('id', 1)->value('image'));
        $this->assertNull(DB::table('resources')->where('id', 2)->value('image'));
        $this->assertEquals('https://example.com/image2.png', DB::table('resources')->where('id', 3)->value('image'));
        $this->assertEquals('https://example.com/image3.png', DB::table('resources')->where('id', 4)->value('image'));
    }
