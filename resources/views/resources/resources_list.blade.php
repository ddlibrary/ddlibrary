                ...
                @foreach ($resources->unique('id') AS $resource)
                    @if ($resource->status)
                        <div class="card resource-card col-8 col-md-4 col-xl-3 col-xxl-2 m-1 p-0">
                            <img class="card-img-top lazyload" data-src="{{ $resource->name ? getResourceImage($resource->namem, true) : getImagefromResource($resource->abstract, '282x254') }}" alt="Resource image" src="">
