...
@section('content')
    <div class="container mt-3">
        ...
        <hr>
        <form method="POST" action="@if($edit){{ route('edit2', $resource['id']) }}@else{{ route('step2') }}@endif" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-item col-6">
                ...
                <select class="form-select{{ $errors->has('subject_areas') ? ' is-invalid' : '' }}" id="subject_areas" name="subject_areas[]" size="10" required multiple>
                    @foreach ($subjects AS $item)
                        @if($item->parent == 0)
                            <optgroup label="{{ $item->name }}">
                                <option value="{{ $item->id }}" {{ $resourceSubjectAreas != null ? (in_array($item->id, $resourceSubjectAreas) ? 'selected' : '') : '' }}>{{ $item->name }}</option>
                                    <?php if (isset($subjects) && isset($item)) {
                                        $parentItems = $subjects->where('parent', $item->id);
                                    }
                                    ?>
                                @foreach($parentItems as $pitem)
                                    <option value="{{ $pitem->id }}" {{ $resourceSubjectAreas != null ? (in_array($pitem->id, $resourceSubjectAreas) ? 'selected' : '') : '' }}>{{ $pitem->name . termEn($pitem->id) }}</option>
                                @endforeach
                            </optgroup>
                        @endif
                    @endforeach
            ...
            <div class="form-item col-6">
                ...
                <input class="form-control{{ $errors->has('keywords') ? ' is-invalid' : '' }}" id="keywords" name="keywords" type="text" value="{{ isset($resource['keywords'])?$resource['keywords']:"" }}" onkeydown="javascript:bringMeAttr('keywords','{{ URL::to('resources/attributes/keywords') }}')">
            ...
            <div class="form-item col-6">
                ...
                <select class="form-control{{ $errors->has('educational_use') ? ' is-invalid' : '' }}" id="educational_use" name="educational_use[]" size="5" required  multiple>
                    @foreach ($educationalUse AS $item)
                        <option value="{{ $item->id }}" {{ $editEducationalUse != null ? (in_array($item->id, $editEducationalUse) ? 'selected' : '') : '' }}>{{ $item->name . termEn($item->id) }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-item">
                ...
                <ul style="list-style-type: none;">
                    ...
                    @foreach($levels AS $level)
                        @if($level->parent == 0)
                            <li><input type="checkbox" name="level[]" {{ $resourceLevels != null ? (in_array($level->id, $resourceLevels ) ? "checked" : "") : ""}} value="{{ $level->id }}" onchange="fnTest(this,'subLevel{{$level->id}}');"> {{ $level->name . termEn($level->id) }}
            ...
            <div class="d-grid gap-2 d-md-block">
                <input class="btn btn-primary btn-md" type="button" value="@lang('Previous') {{ en('Previous') }}" onclick="location.href='@if($edit){{ route('edit1', $resource['id']) }}@else{{ route('step1') }}@endif'">
