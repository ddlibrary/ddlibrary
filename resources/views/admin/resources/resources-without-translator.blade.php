    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Resources without translator</li>
            </ol>
            
            <div class="pb-4">
                <form method="get" action="{{ route('resource-without-translators') }}">
                    @csrf
                    <div class="row">
                        ...
                        {{-- Without translator --}}
                        <div class="col-md-2">
                            <label>Without translator</label>
                            <select class="form-control" name="without_translator">
                                <option value="">...</option>
                                <option value="1"
                                    {{ isset($filters['without_translator']) && $filters['without_translator'] == 1 ? 'selected' : '' }}>
                                    Yes</option>
                                        ...
                                        <td class="w-50">
                                            <div>
                                                <div class="row" style="width: 100%">
                                                    <div class="col-9">
                                                        <input type="text" value="{{ $resource->translators->pluck('name')->implode(', '); }}" class="form-control make-disable item-{{$resource->id}}"  placeholder="Please add translator">
                                                    </div>
                                                    <div class="col-3">
                                                        <button class="btn btn-success make-disable" onclick="addTranslator({{ $resource->id }})">Save</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
        ...
        <script>
            function addTranslator(resourceId) {
            const name = $(`.item-${resourceId}`).val();
            if(confirm('Are you sure to change the translator?')){
                if (name) {
                    $('.make-disable').attr('disabled', true)
                    $.ajax({
                        url: "{{ url('admin/resources/add-translator') }}",
                        method: 'POST',
                        data: {
                            name: name,
                        ...
                        error: function(xhr) {
                            const errorMessage = xhr.responseJSON?.message || 'Failed to add translator.';
                    ...
                    showToast('Please enter a translator name.', 'error');
                }
            }
        }
        </script>
