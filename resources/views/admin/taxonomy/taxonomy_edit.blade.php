            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <a href="{{ URL::to('admin/taxonomy/translate/' . $term->id) }}"><button
                                class="btn btn-primary pull-right">Translate</button></a>
                        <form method="POST" action="{{ url('admin/taxonomy/update/' . $term->vid . '/' . $term->id) }}">
                            ...
                            <table class="table table-bordered" width="100%" cellspacing="0">
                                ...
                                <tr>
                                    <td>Name</td>
                                    <td>
                                        <input class="form-control" type="text" value="{{ $term->name }}"
                                            name="name" required>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <select class="form-control" name="parent" required>
                                            ...
                                            @foreach ($parents as $p)
                                                <option value="{{ $p->id }}"
                                                    {{ $p->id == $theParent ? 'selected' : '' }}>{{ $p->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Weight</td>
                                    <td>
                                        <input class="form-control" type="text" value="{{ $term->weight }}"
                                    ...
                                    <td>
                                        <select class="form-control" name="language" required>
                                            ...
                                            @foreach (LaravelLocalization::getSupportedLocales() as $localcode => $properties)
                                                <option value="{{ $localcode }}"
                                                    {{ $term->language == $localcode ? 'selected' : '' }}>
