...
<section class="ddl-forms">
    ...
    <div class="content-body">
        ...
        <form method="POST" action="{{ route('add_page_translate', ['pageid' => $tnid, 'lang' => $lang]) }}">
        @csrf
        <div class="form-item">
            <label for="title"> 
        ...
        <div class="form-item">
            ...
            <select class="form-control{{ $errors->has('language') ? ' is-invalid' : '' }}" name="language" id="language" required disabled>
                <option value="">- @lang('None') -</option>
                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                <option value="{{ $localeCode }}" {{ $lang == $localeCode ? "selected" : "" }}>{{ $properties['native'] }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-item">
